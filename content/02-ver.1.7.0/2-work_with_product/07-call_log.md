...
В этом поле отображается результат выполнения сервисов АД и АППС.  
Для АППС:  
...
🛈 Для **Автообзвона должников** и **Приема показаний** один и тот же номер привязывается по разному. Для **Автообзвона должников** начинается на **8**, для **Приема показаний** начинается на **7**.
